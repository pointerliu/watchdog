...
use crate::domains::fetcher::api::handlers::{
    add_fetcher, get_fetcher_types, get_user_fetchers, remove_fetcher,
};
...
pub fn fetcher_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/fetchers/types", web::get().to(get_fetcher_types))
        ...
        .route(
            "/fetchers/{user_id}/{fetcher_name}",
            web::delete().to(remove_fetcher),
        );
}
