...
use crate::domains::subscription::dto::subscription_dto::{CreateSubscriptionRequest, SubscriptionResponse};
use crate::common::bootstrap::{SimpleSubscriptionCriteria, FetchedDataStorage};
...
pub async fn create_subscription(
    ...
    info!("Creating subscription for user: {}, criteria: {}", req.user_id, req.criteria_id);
    ...
    match data.watchdog.add_subscription(subscription).await {
        ...
        Err(e) => {
            let api_response: ApiResponse<SubscriptionResponse> = ApiResponse::error(500, format!("Failed to create subscription: {}", e));
...
pub async fn remove_subscription(
    ...
    match data.watchdog.remove_subscription(&criteria_id).await {
        Ok(Some(_)) => {
            let response: ApiResponse<()> = ApiResponse::success_with_message("Subscription removed successfully".to_string());
            HttpResponse::Ok().json(response)
        }
        Ok(None) => {
            let response: ApiResponse<()> = ApiResponse::success_with_message("Subscription not found".to_string());
            HttpResponse::Ok().json(response)
        }
        Err(e) => {
            let response: ApiResponse<()> = ApiResponse::error(500, format!("Failed to remove subscription: {}", e));
